...
export const signUpForRewards = async (wallet) => {
  ...
  if (wallet.type == 'wallet-connect') {
    return await SignWalletConnectAndSendTxns(wallet.connector, algodClient, txn, text)
  } else {
    return await SignMyAlgoAndSendTxns(algodClient, txn)
  }
}

const SignWalletConnectAndSendTxns = async (connector, algodClient, txn, text) => {
  try {
    ...
    console.log({ request })
    // console.log(await connector?.sendCustomRequest())
